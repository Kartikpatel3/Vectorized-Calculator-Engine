...
class performance_table:
    ...
    def Mat_Mul(self, first, second):
        ...
        """
        Native_time = vec_py.matrix_multiplication(first, second)
        Numpy_time = vec_np.matrix_multiplication(np.array(first), np.array(second))
        if isinstance(Native_time, (int, float)):
            arr = np.array(first)
            size = arr.size
            Speedup = round(Native_time / Numpy_time , 0)
